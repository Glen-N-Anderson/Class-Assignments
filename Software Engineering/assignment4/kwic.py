...
#Below are helper functions that do not need to be in the class. They mostly just strip and shift words. 
def shift(line):
...
class Kwic:
	...
	def index(self):
		...
		shiftedLines = [map(lambda x:(x,i), shift(splitLines[i])) for i in range(0,len(splitLines))]
    		...
    		filteredLines = filter(lambda l: not ignorable(l[0][0], self.ignoreWords), flattenedLines)
		es.event("callIndex")
