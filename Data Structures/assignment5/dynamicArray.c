...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
	if(heap != NULL){
		int loc = sizeDynArr(heap) - 1;
		addDynArr(heap, val);
		while(loc > 0){ //go through entire tree in case it is smaller than higher order parents (grandparents etc)
			int par; 
			par = (loc - 1) / 2; //parent defined using equation from slides in lecture
			if(compare(getDynArr(heap, loc), getDynArr(heap, par)) == -1){ //if the child is smaller than parent, we must swap
				swapDynArr(heap, loc, par);
				loc = par;
			}
			else {
				break;
...
{
  /* FIXME: Write this */
	int lChild = (2 * pos) + 1; //position is located on row above children, so we must mult by two to get their position in next row
	int rChild = (2 * pos) + 2;
	//must keep in mind that children must be < max to run algorithm (NOT equal to)
	//this will run until heap is in proper order (until it is adjusted up to max)
	if(rChild < max){ 
		if(_smallerIndexHeap(heap, lChild, rChild, compare) == lChild){ //if left child is smaller, it must be swapped first as it has priority
			if(_smallerIndexHeap(heap, pos, lChild, compare) == lChild){ //if smaller than position, swap
				swapDynArr(heap, lChild, pos);
				_adjustHeap(heap, max, lChild, compare);
			
			}
		}
		else if(_smallerIndexHeap(heap, lChild, rChild, compare) == rChild){
			if(_smallerIndexHeap(heap, pos, rChild, compare) == rChild){
				swapDynArr(heap, rChild, pos);
				_adjustHeap(heap, max, rChild, compare);
			}
		}	
	}		
	else if(lChild < max){
		if(_smallerIndexHeap(heap, pos, lChild, compare) == lChild){
			swapDynArr(heap, lChild, pos);
			_adjustHeap(heap, max, lChild, compare);
		}
...
void _buildHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write This */
	int i;
	int parent = (heap->size / 2) - 1; //gives us first parent for heap
	for(i = parent; i >= 0; i--){
...
{
  /* FIXME: Write this */
	//can use similar algorithm to _adjustHeap (in part by calling it)
	if(heap->size != 0){
		int i = 0; 
	  	 _buildHeap(heap, compare);
		for(i = heap->size - 1; i > 0; i--){ //use decrementing loop instead of incrementing, better for this structure
			swapDynArr(heap, 0, i);
			_adjustHeap(heap, i, 0, compare); //set current place as max in adjustheap to sort to that point
