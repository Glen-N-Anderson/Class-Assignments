...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	if(tree != NULL){    	
		...
		while(itr != 0){
			if(compare(val, itr->val) == 1){
				itr = itr->right;
			}
			else if(compare(val, itr->val) == -1){
				itr = itr->left;
			}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	if(cur != NULL){
		...
		cur->left = _removeLeftMost(cur->left); //left child isn't null
...
{
    /*write this*/
    	//must find val to remove by searching, use compare to do this
    	if(cur != NULL){
		if(val != NULL){
			...
			if(compare(val, cur->val) == 1){
				...
				if(cur->right == 0 && cur->left == 0){
					...
					struct Node *remNode = cur->left;
					free(cur);
					return remNode;
				}
				else if(cur->left == 0){
					struct Node *remNode = cur->right;
					free(cur);
					return remNode;
				}
				else{
